...
const FIX_INSTRUCTIONS: Record<string, string> = {
  ...
  "hardcoded-color": 'Check each node\'s matchName/matchId for a suggested style or variable. If a match exists: frames(method:"update", items:[{id, fill:{styleName:"..."}}]) or bind a variable via items:[{id, bindings:[{field:"fills/0/color", variableId:"..."}]}]. If no match: create a style with styles(method:"create", type:"paint") or a variable with variables(method:"create") first, then apply it.',
