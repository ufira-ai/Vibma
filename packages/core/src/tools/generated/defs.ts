...
export const tools: ToolDef[] = [
  ...
  },
  {
    ...
    offset: z.coerce.number().optional().default(0).describe("Skip N items for pagination (default 0)"),
    limit: z.coerce.number().optional().default(100).describe("Max items per page (default 100)"),
    items: flexJson(z.array(z.record(z.string(), z.unknown()))).optional().describe("Array of {id, ...properties} to update/delete/clone/reparent"),
    name: z.string().optional().describe("Rename the clone (set before appending to parent — required when cloning a variant into its component set to avoid duplicate names)"),
    x: z.coerce.number().optional().describe("X position (default: 0)"),
    y: z.coerce.number().optional().describe("Y position (default: 0)"),
    rules: flexJson(z.array(z.string())).optional().describe("Rules to check. Default: [\"all\"]. Categories: \"component\", \"composition\", \"token\", \"naming\", \"wcag\"."),
  ...
  },
  {
    ...
    items: flexJson(z.array(z.record(z.string(), z.unknown()))).optional().describe("Array of {id, ...properties} to delete/clone/reparent/create/update/swap/detach/reset_overrides"),
    name: z.string().optional().describe("Rename the clone (set before appending to parent — required when cloning a variant into its component set to avoid duplicate names)"),
    x: z.coerce.number().optional().describe("X position (default: 0)"),
    y: z.coerce.number().optional().describe("Y position (default: 0)"),
    depth: z.coerce.number().optional().describe("Response detail: omit for id+name only. 0=properties + child stubs. N=recurse N levels. -1=unlimited."),
    ...
    validate: (params: any) => {
      ...
      if (m === "create") {
        ...
        const itemSchema = z.object({
          ...
          minHeight: z.coerce.number().optional().describe("Min height for responsive auto-layout"),
          maxHeight: z.coerce.number().optional().describe("Max height for responsive auto-layout"),
          componentKey: z.string().optional().describe("Published component key hash (40-char hex) for cross-file library import. Preferred over componentId for library components."),
          componentId: z.string().optional().describe("Local component or component set node ID (e.g. '1:23'). Use componentKey instead for cross-file library components."),
      ...
      if (m === "update") {
        const itemSchema = z.object({
          ...
          textStyleKey: z.string().optional().describe("Published text style key hash (40-char hex) for cross-file library import. Preferred over textStyleId for library styles."),
          textStyleId: z.string().optional().describe("Apply text style by ID — overrides fontSize/fontWeight. Use textStyleKey instead for cross-file library styles."),
      ...
      if (m === "swap") {
        const itemSchema = z.object({
          id: z.string().describe("Instance node ID"),
          componentKey: z.string().optional().describe("Published component key hash (40-char hex) for cross-file library import. Preferred over componentId for library components."),
          componentId: z.string().optional().describe("Local component or component set node ID"),
          variantProperties: z.record(z.string(), z.unknown()).optional().describe("Pick variant e.g. {\"size\":\"24\",\"style\":\"regular\"}"),
        }).passthrough();
        try { params.items = z.array(itemSchema).parse(params.items); }
        catch (e) { if (e instanceof z.ZodError) { throw new Error(e.issues.map(i => { const path = i.path.join("."); const shape = itemSchema instanceof z.ZodObject ? (itemSchema as any).shape : null; const desc = shape?.[i.path[1]]?.description; return path + ": " + i.message + (desc ? " (expected: " + desc + ")" : ""); }).join("; ")); } throw e; }
      }
      ...
      if (m === "create") {
        const schemas: Record<string, z.ZodTypeAny> = {
          "paint": z.object({
            ...
            color: S.colorRgba.optional().describe("Color value. Optional when colorVariableName is provided."),
            colorVariableName: z.string().optional().describe("Bind to a COLOR variable by name (style tracks the variable). Can be used alone — color is resolved from the variable."),
  ...
  },
  {
    ...
    offset: z.coerce.number().optional().default(0).describe("Skip N items for pagination (default 0)"),
    limit: z.coerce.number().optional().default(100).describe("Max items per page (default 100)"),
    items: flexJson(z.array(z.record(z.string(), z.unknown()))).optional().describe("Array of {id, ...properties} to update/delete/clone/reparent/create/set_content/scan"),
    name: z.string().optional().describe("Rename the clone (set before appending to parent — required when cloning a variant into its component set to avoid duplicate names)"),
    x: z.coerce.number().optional().describe("X position (default: 0)"),
    y: z.coerce.number().optional().describe("Y position (default: 0)"),
    rules: flexJson(z.array(z.string())).optional().describe("Rules to check. Default: [\"all\"]. Categories: \"component\", \"composition\", \"token\", \"naming\", \"wcag\"."),
    ...
    validate: (params: any) => {
      ...
      if (m === "create") {
        ...
        const itemSchema = z.object({
          ...
          fontColorStyleName: z.string().optional().describe("Apply paint style — overrides fontColor"),
          textStyleKey: z.string().optional().describe("Published text style key hash (40-char hex) for cross-file library import. Preferred over textStyleId for library styles."),
          textStyleId: z.string().optional().describe("Local text style ID or name (case-insensitive) — overrides fontSize/fontWeight. Use textStyleKey instead for cross-file library styles."),
