...
export const tools: ToolDef[] = [
  ...
  },
  {
    ...
    factor: z.coerce.number().optional().describe("Uniform scale factor, >= 0.01. 0.5 = 50%, 2 = 200%. Uses Figma's native rescale behavior."),
    scaleX: z.coerce.number().optional().describe("Horizontal scale factor, >= 0.01. Use with scaleY for independent axis scaling. Omitted axis defaults to 1 unless lockAspectRatio:true."),
    scaleY: z.coerce.number().optional().describe("Vertical scale factor, >= 0.01. In independent axis scaling, text, strokes, corners, and effect radii follow this vertical scale."),
    lockAspectRatio: z.boolean().optional().describe("When true with scaleX/scaleY, keep proportions by using a uniform factor. If both axes are provided they must match. factor is always uniform."),
