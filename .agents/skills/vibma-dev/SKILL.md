...
- **Validation** — one validator per concern, composed where needed; schema-derived validation belongs in `schema/compiler`, batch-shape validation belongs in `batchHandler`
...
- If the issue involves `items`, empty arrays, discriminants (`type`), examples, or unknown-param messages, audit all endpoints using the same generated schema or `batchHandler` path before patching one case
...
- Put examples in `schema/tools/*.yaml`; put schema-derived validation in `schema/compiler`; put adapter batch behavior in `batchHandler`; only put resource-specific constraints in handlers

### 5. Test directly with Vibma MCP

Test extensively using the MCP tools, not just by reading the code. Use `dev_reload` to pick up changes without restarting Codex.
...
- For shared batch or discriminant fixes, test at least one unrelated endpoint that uses the same path (e.g. components + frames + styles, or create + update/delete batches)
